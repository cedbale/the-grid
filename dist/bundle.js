...
},{"./lib/grid.js":2}],2:[function(require,module,exports){
'use strict';
...
function countActivesSquares(position) {
    let sum = [isSquareActivated(position - lineNb), // top
...
function isASquareBorder(position) {
    return position !== 1 && position !== totalSquare && (position % lineNb === 0 || (position - 1) % lineNb === 0);
}
...
function run(cbToDisplay) {
    ...
    _(grid).flatten().forEach((square, idx) => {
        square.colorize();
        if (cbToDisplay) {
            output.push(square[cbToDisplay]());
            if ((idx + 1) % lineNb === 0) {
                output.push('\n');
            }
        }
